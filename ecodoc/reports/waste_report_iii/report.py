    ...
    def _general(self, wb):
        ...
        rows = [
            ("ОТЧЁТНОСТЬ ОБ ОБРАЗОВАНИИ, УТИЛИЗАЦИИ, ОБЕЗВРЕЖИВАНИИ, "
             "РАЗМЕЩЕНИИ ОТХОДОВ", ""),
            ("⚠ Отдельной федеральной формы для этой отчётности НЕТ: "
             "приказ Минприроды № 30 отменён с 01.01.2021.", ""),
            ("Объекты III категории отчитываются в составе отчёта ПЭК "
             "(приказ Минприроды от 15.03.2024 № 173, раздел 4);", ""),
            ("субъекты МСП регионального надзора — по региональному порядку. "
             "Этот документ — рабочая сводка, а не бланк подачи.", ""),
            ("", ""),
