...
# Разделы расчёта по форме декларации. Р1–Р9 — Приказ № 1043 в ред. № 241
# от 29.04.2025 (за 2025 год); Р10 (искусственные грунты, признанные отходами)
# появился в форме Приказа Минприроды от 01.04.2026 № 182 — она применяется
# с 01.09.2026, т.е. начиная с декларации за 2026 год. Какие разделы печатать,
# решает report.py по отчётному году; расчёт держит все десять, чтобы плата
# по «грунтовым» отходам не терялась в общем итоге.
SECTIONS = {
    ...
    "Р10": "Размещение, в том числе складирование, искусственных грунтов, "
           "признанных отходами",
}
_WASTE_SECTION = {"prod": "Р5", "tko": "Р6", "byproduct": "Р7",
                  "overburden": "Р8", "livestock": "Р9", "soil": "Р10"}
...
class PayLine:
    ...
    section: str = "Р1"  # раздел декларации Р1..Р10
...
class PaymentResult:
    ...
    total_waste: Decimal = Decimal("0")   # Р5+Р6+Р7+Р8+Р9+Р10 (все отходы)
...
def calculate(ctx: ReportContext) -> PaymentResult:
    ...
    if not year:
        res.warnings.append(
            ...
            f"{k_ind}. Заполните год во вкладке ОТЧЁТНОСТЬ: от него зависят ставки "
