...
(дополнительно сверен с принятым отчётом ПРОТЕЛЮКС за 2023 — скан
«Формы/Отчетность/2-ТП водхоз/ПРОТЭЛЮКС 2-ТП водхоз 2024 скан.pdf», 4 стр.:
титул с кодовой частью и «Бланк № 1 / Всего бланков 1», Раздел 1 ЧЕТЫРЬМЯ
блоками граф 1-6 / 7-19 / 20-31 / 32-49 с предпечатанными строками 11-15 и
подписями «Код по ОКЕИ: километр - 008» и «…тысяча кубических метров -
114», Раздел 2 блоками 1-6 / 7-18 / 19-30 и три блока по 8 пар ЗВ
(31-46 / 47-62 / 63-78) со строками 21-25, сноска <1> и примечание, блок
подписи с датой «__ 20__ год». У респондента заполнена только строка 11
Раздела 1 (забор по лицензии Л 480082 из БАЛ/НАРВА, код 60, ТН, 1,29 тыс м³
по месяцам, гр.32-33 «102 / 1,29»), Раздел 2 — пустые предпечатанные строки.
Поэтому кроме машинных листов «Раздел 1/2» (графы подряд) печатаются листы
«Раздел 1 (бланк)» / «Раздел 2 (бланк)» в той же разбивке на блоки, что и
сданный отчёт):
...
class TP2Water(Report):
    ...
    def _title_page(self, wb, w):
        ...
        xlsx.merge(ws, "A24:F24", "(номер контактного телефона)            "
                   ...
                   align="left")
        # «Бланк № 1 / Всего бланков 1» и «Годовая» — стоят на каждой
        # странице сданного отчёта ПРОТЕЛЮКС-2023; у нас один бланк
        xlsx.merge(ws, "A26:F26", "Бланк №  1        Всего бланков  1        "
                   "Годовая", border=False, align="left")
        xlsx.heights(ws, {3: 30, 15: 90})

    # --- печатная разбивка по бланку --------------------------------------
    # Раздел 1 сданного отчёта: 4 блока граф; над 1-м «Код по ОКЕИ: километр -
    # 008», над 2-м «…тысяча кубических метров - 114» (скан стр. 2)
    _S1_BLOCKS = [((1, 6), "Код по ОКЕИ: километр - 008"),
                  ((7, 19), "Код по ОКЕИ: тысяча кубических метров - 114"),
                  ((20, 31), ""), ((32, 49), "")]
    # Раздел 2 (скан стр. 3-4): 3 блока граф + пары ЗВ по 8 пар на блок
    _S2_BLOCKS = [((1, 6), "Код по ОКЕИ: километр - 008"),
                  ((7, 18), "Код по ОКЕИ: тысяча кубических метров - 114"),
                  ((19, 30), "")]
    PREPRINTED_ROWS = 5   # строки 11-15 / 21-25 предпечатаны в бланке

    def _blank_sheet(self, wb, name, title, mark, cols, blocks, first_row_no,
                     rows, tail_lines=()):
        """Лист «как на печати»: блоки граф бланка один под другим, в каждом —
        подпись ОКЕИ, шапка, строка номеров граф, предпечатанные строки
        (11-15 / 21-25: в сданном отчёте пустые строки тоже напечатаны), внизу
        «Бланк № 1 / Всего бланков 1». Значения те же, что на машинном листе."""
        ws = wb.create_sheet(name)
        by_num = {num: lbl for num, lbl in cols}
        vals_by_row = {}
        for n, (_lead, vals) in enumerate(rows):
            vals_by_row[n] = {num: v for (num, _l), v in zip(cols, vals)}
        nrows = max(len(rows), self.PREPRINTED_ROWS)
        xlsx.cell(ws, "A1", mark, bold=True, border=True, size=10)
        xlsx.merge(ws, "B1:L1", title, bold=True, border=False, align="left")
        r = 3
        width = 0
        for (a, b), okei in blocks:
            nums = [n for n in range(a, b + 1) if n in by_num]
            if not nums:
                continue
            width = max(width, len(nums) + 1)
            last = get_column_letter(1 + len(nums))
            if okei:
                xlsx.merge(ws, f"A{r}:{last}{r}", okei, border=False,
                           italic=True, align="right", size=9)
            r += 1
            xlsx.cell(ws, f"A{r}", "N строки", bold=True, fill=True, size=8)
            xlsx.cell(ws, f"A{r + 1}", "А", italic=True, size=8)
            for i, num in enumerate(nums):
                c = get_column_letter(2 + i)
                xlsx.cell(ws, f"{c}{r}", by_num[num], bold=True, fill=True, size=8)
                xlsx.cell(ws, f"{c}{r + 1}", num, italic=True, size=8)
            xlsx.heights(ws, {r: 64})
            r += 2
            for n in range(nrows):
                xlsx.cell(ws, f"A{r}", first_row_no + n)
                data = vals_by_row.get(n, {})
                for i, num in enumerate(nums):
                    v = data.get(num)
        ...
        for line in tail_lines:
            xlsx.merge(ws, f"A{r}:P{r}", line, border=False, italic=True,
                       align="left")
            r += 1
        xlsx.merge(ws, f"A{r + 1}:H{r + 1}", "Бланк №  1        Всего бланков  1",
                   border=False, align="right")
        xlsx.widths(ws, {"A": 9, **{get_column_letter(2 + i): 11
    ...
    def _section1(self, wb, w):
        ...
        title = ("Раздел 1. Забрано из природных источников, получено от "
                 "поставщиков, использовано, передано и потеряно воды")
        self._numbered_sheet(wb, "Раздел 1", title, _S1_COLS, 11, rows)
        self._blank_sheet(wb, "Раздел 1 (бланк)", title, "Т1", _S1_COLS,
                          self._S1_BLOCKS, 11, rows)
    ...
    def _section2(self, wb, w):
        ...
        tail = ("<1> БПК полн (132), взвешенные вещества (113), "
        ...
        xlsx.merge(ws, f"A{r + 2}:P{r + 2}", tail[0], border=False,
                   italic=True, align="left")
        xlsx.merge(ws, f"A{r + 3}:P{r + 3}", tail[1], border=False,
                   italic=True, align="left")
        # печатная разбивка: в сданном отчёте пары ЗВ идут блоками по 8 пар
        # (гр. 31-46 / 47-62 / 63-78), и даже без ЗВ печатается ОДИН пустой
        # блок пар — как на стр. 4 скана ПРОТЕЛЮКС
        blocks = list(self._S2_BLOCKS)
        zv_last = 30 + 2 * max(npairs, 1)
        for a in range(31, zv_last + 1, 16):
            blocks.append(((a, min(a + 15, zv_last)), ""))
        if npairs == 0:
            cols += [(31, "Содержание ЗВ в отведённых водах — код ЗВ (Прил. 5)"),
                     (32, "Содержание ЗВ в отведённых водах — масса (т/кг по Прил. 5)")]
        self._blank_sheet(wb, "Раздел 2 (бланк)", "Раздел 2. Водоотведение",
                          "Т2", cols, blocks, 21, rows, tail_lines=tail)
