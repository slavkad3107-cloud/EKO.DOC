...
class DVOS(_Dev):
    code = "dvos"
    # ДВОС подаётся в РПН как отчётный документ (раз в 7 лет), а не
    # разрабатывается «в стол» — по замечанию пользователя карточка живёт в
    # «Отчётности»; генератор при этом тот же (.docx по пр. № 117)
    domain = "reporting"
