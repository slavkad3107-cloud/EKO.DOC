...
# Третий элемент (необязательный) — КОНКРЕТНЫЙ вопрос пользователю, если поля
# нет в документах: «только конкретно, что вводить надо запрашивать» (ТЗ).
_BASE_FIELDS = [
    ("organization.name", "наименование организации",
     "Введите полное наименование организации, как в ЕГРЮЛ"),
    ("organization.inn", "ИНН",
     "Введите ИНН: 10 цифр у юридического лица, 12 у ИП"),
    ("organization.oktmo", "ОКТМО",
     "Введите ОКТМО места внесения платы — 8 или 11 цифр "
     "(рядом есть кнопка «Определить по адресу»)"),
    ("period.year", "отчётный год", "Введите отчётный год, например 2025"),
]
...
REQUIREMENTS: dict[str, dict] = {
    ...
    "4-oos": {
        "fields": _BASE_FIELDS + [
            ("extra.oos4.costs", "текущие затраты на охрану окружающей среды "
                                 "по направлениям (вкладка «Отчётность» → «Общая»)")],
        "docs": _BASE_DOCS + [
            "данные бухгалтерии о текущих затратах на ООС за год",
            "платёжные документы по плате за НВОС (для строки платы)",
        ],
    },
    "2tp-waste": {
...
def check(ctx: ReportContext, form: str) -> tuple[list[str], list[str]]:
    ...
    if not req:
        # не молчим: пустой ответ читался как «данных достаточно», и опечатка
        # в коде формы выглядела как готовность к сдаче
        return [f"требования к форме «{form}» в программе не описаны — "
                f"проверьте комплектность вручную"], []
    missing = []
    # у части полей задан третий элемент — вопрос пользователю; старые
    # двухэлементные записи продолжают работать
    for path, label, *_q in req["fields"]:
