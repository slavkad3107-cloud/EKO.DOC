...
def act_plausibility_problem(act) -> str:
    """Сходятся ли масса и объём акта.

    В базе Миниха: 0,002 т при 2 м³ (плотность 0,001 т/м³) и 6 602 т при
    352 224 м³ — так не бывает ни у одного отхода: при распознавании таблицы
    перепутаны графы «т» и «м³» либо единицы. Диапазон 0,05–8 т/м³ покрывает
    всё от плёнки до металла."""
    try:
        m, v = float(act.mass or 0), float(act.volume_m3 or 0)
    except (TypeError, ValueError):
        return ""
    if m > 0 and v > 0:
        rho = m / v
        if rho < 0.05 or rho > 8:
            return (f"масса и объём не сходятся (плотность {rho:.3f} т/м³) — "
                    f"проверьте по документу, не перепутаны ли т и м³")
    if m > 5000:
        return f"масса {m:g} т за один акт неправдоподобна — проверьте единицы (кг? м³?)"
    return ""
