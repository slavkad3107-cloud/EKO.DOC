...
class WasteFlow:
    ...
    transferred_processing: Decimal = Decimal("0")  # из переданного — на обработку
    ...
    # промышленность, перерабатывающая и прочие. Пусто = «прочие».
    industry: str = ""          # «добывающая» | «перерабатывающая» | ""
    # стимулирующий коэффициент Кст к ставке за размещение (ст. 16.3 ФЗ-7):
    # 0.3 — размещение в пределах лимитов отходов IV–V кл., образованных при
    # утилизации/обезвреживании; 0 — утилизация в собственном производстве /
    # вскрышные породы V кл. Пусто (None) => 1 (не применяется).
    k_st: Optional[Decimal] = None
...
class WasteAct:
    ...
    carrier_license: str = ""   # реквизиты лицензии перевозчика
    receiver: str = ""          # приёмщик / полигон / получатель
    receiver_inn: str = ""
    license: str = ""           # реквизиты лицензии получателя (полигона)
    date: str = ""              # дата акта (ДД.ММ.ГГГГ) — как в документе
    # период акта — то, по чему строятся месяц/квартал/год: в справках
    # операторов вместо даты часто стоит «3 кв 2025» или «март 2025», и
    # эколог хочет видеть разбивку по годам → кварталам → месяцам.
    # 0 — неизвестно. Заполняется из date/периода при приёме и при вводе.
    year: int = 0
    quarter: int = 0            # 1..4
    month: int = 0              # 1..12
