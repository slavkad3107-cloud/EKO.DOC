...
OBLIGATIONS: list[Obligation] = [
    ...
    Obligation(
        ...
        where="Росводресурсы/ЛКПП", coverage="за предыдущий год", due=[(1, 22)]),
    Obligation(
        "waste-report-iii",
        "Отчётность об образовании, утилизации… отходов (III кат./МСП)",
        "reporting", "periodic", "год", "Приказ Минприроды №1028/№30",
        lambda p: p.cat_in("III") or p.is_msp,
        coverage="за предыдущий год", due=[(1, 25)]),
    Obligation(
    ...
    Obligation(
        ...
        "periodic", "год", "ст. 16.4 ФЗ-7; Приказ Минприроды №1043",
    ...
    Obligation(
        ...
        "periodic", "год", "ст. 67 ФЗ-7; Приказ Минприроды №109",
        _I_III, coverage="за предыдущий год", due=[(3, 25)]),
    Obligation(
        "cadastre", "Региональный кадастр отходов", "reporting", "periodic",
        "год", "региональный НПА (СПб/ЛО)",
        lambda p: p.has_waste and bool(p.region_codes & {"78", "47", "40"}),
        where="региональный портал",
        coverage="за предыдущий год; СРОК РЕГИОНАЛЬНЫЙ — проверить",
        due=[(4, 1)]),
    Obligation(
    ...
    Obligation(
        ...
        "reporting", "periodic", "квартал", "Приказ Минприроды №1028",
